/*
MUDBASESDK

MUDBASE is a scalable, real-time, and secure Backend-as-a-Service (BaaS) platform  designed for modern applications. Built with custom logic, it offers fine-grained  control, extensibility, and enterprise-grade security.  ## Features - 🔐 Multi-provider authentication (30+ OAuth providers) - 📊 Real-time database with collections - 📁 File storage and management - 🔑 API key management with permissions - 🔗 Webhook system with retry logic - ⚡ Serverless functions - 💬 Multi-channel messaging (Push, Email, SMS) - 📈 Usage analytics and monitoring - 🌐 Real-time WebSocket events - 🔍 Full-text search capabilities - 💳 Billing: Flutterwave only for fiat (platform subscriptions + org payment processing with 7% + $0.50 split); crypto payment processor for multi-chain checkout - 📡 Block-based multi-chain wallet monitoring (ETH/UTXO scanners, GetBlock, scanner metrics API) 

API version: 1.2.3
Contact: support@dev.mudbase
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package mudbase

import (
	"encoding/json"
)

// checks if the WalletGetTransactionByHash200Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &WalletGetTransactionByHash200Response{}

// WalletGetTransactionByHash200Response struct for WalletGetTransactionByHash200Response
type WalletGetTransactionByHash200Response struct {
	Success *bool `json:"success,omitempty"`
	Data *WalletTransaction `json:"data,omitempty"`
}

// NewWalletGetTransactionByHash200Response instantiates a new WalletGetTransactionByHash200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewWalletGetTransactionByHash200Response() *WalletGetTransactionByHash200Response {
	this := WalletGetTransactionByHash200Response{}
	return &this
}

// NewWalletGetTransactionByHash200ResponseWithDefaults instantiates a new WalletGetTransactionByHash200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewWalletGetTransactionByHash200ResponseWithDefaults() *WalletGetTransactionByHash200Response {
	this := WalletGetTransactionByHash200Response{}
	return &this
}

// GetSuccess returns the Success field value if set, zero value otherwise.
func (o *WalletGetTransactionByHash200Response) GetSuccess() bool {
	if o == nil || IsNil(o.Success) {
		var ret bool
		return ret
	}
	return *o.Success
}

// GetSuccessOk returns a tuple with the Success field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WalletGetTransactionByHash200Response) GetSuccessOk() (*bool, bool) {
	if o == nil || IsNil(o.Success) {
		return nil, false
	}
	return o.Success, true
}

// HasSuccess returns a boolean if a field has been set.
func (o *WalletGetTransactionByHash200Response) HasSuccess() bool {
	if o != nil && !IsNil(o.Success) {
		return true
	}

	return false
}

// SetSuccess gets a reference to the given bool and assigns it to the Success field.
func (o *WalletGetTransactionByHash200Response) SetSuccess(v bool) {
	o.Success = &v
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *WalletGetTransactionByHash200Response) GetData() WalletTransaction {
	if o == nil || IsNil(o.Data) {
		var ret WalletTransaction
		return ret
	}
	return *o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WalletGetTransactionByHash200Response) GetDataOk() (*WalletTransaction, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *WalletGetTransactionByHash200Response) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given WalletTransaction and assigns it to the Data field.
func (o *WalletGetTransactionByHash200Response) SetData(v WalletTransaction) {
	o.Data = &v
}

func (o WalletGetTransactionByHash200Response) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o WalletGetTransactionByHash200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Success) {
		toSerialize["success"] = o.Success
	}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	return toSerialize, nil
}

type NullableWalletGetTransactionByHash200Response struct {
	value *WalletGetTransactionByHash200Response
	isSet bool
}

func (v NullableWalletGetTransactionByHash200Response) Get() *WalletGetTransactionByHash200Response {
	return v.value
}

func (v *NullableWalletGetTransactionByHash200Response) Set(val *WalletGetTransactionByHash200Response) {
	v.value = val
	v.isSet = true
}

func (v NullableWalletGetTransactionByHash200Response) IsSet() bool {
	return v.isSet
}

func (v *NullableWalletGetTransactionByHash200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableWalletGetTransactionByHash200Response(val *WalletGetTransactionByHash200Response) *NullableWalletGetTransactionByHash200Response {
	return &NullableWalletGetTransactionByHash200Response{value: val, isSet: true}
}

func (v NullableWalletGetTransactionByHash200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableWalletGetTransactionByHash200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


