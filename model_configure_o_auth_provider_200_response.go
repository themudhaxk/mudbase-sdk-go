/*
MUDBASESDK

MUDBASE is a scalable, real-time, and secure Backend-as-a-Service (BaaS) platform  designed for modern applications. Built with custom logic, it offers fine-grained  control, extensibility, and enterprise-grade security.  ## Features - 🔐 Multi-provider authentication (30+ OAuth providers) - 📊 Real-time database with collections - 📁 File storage and management - 🔑 API key management with permissions - 🔗 Webhook system with retry logic - ⚡ Serverless functions - 💬 Multi-channel messaging (Push, Email, SMS) - 📈 Usage analytics and monitoring - 🌐 Real-time WebSocket events - 🔍 Full-text search capabilities - 💳 Billing: fiat only for project subscriptions and org BaaS checkout (platform fee split). On-chain billing is not exposed on these APIs (optional `crypto-payment-module/` in repo, not mounted by default). - 📡 Block-based multi-chain wallet monitoring (ETH/UTXO scanners, GetBlock, scanner metrics API) - 🏢 Enterprise / Phase 4: custom domains on Growth, Scale, and Enterprise (TXT DNS at `_mudbase-verify.<hostname>`); `settings.customDomainAddon` is optional (billing/legacy); dedicated DB migration script, periodic DNS recheck job, optional `infrastructureEnvironments[]` and edge/metering fields on `dedicated`. 

API version: 1.3.10
Contact: support@mudbase.dev
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package mudbase

import (
	"encoding/json"
)

// checks if the ConfigureOAuthProvider200Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ConfigureOAuthProvider200Response{}

// ConfigureOAuthProvider200Response struct for ConfigureOAuthProvider200Response
type ConfigureOAuthProvider200Response struct {
	Message *string `json:"message,omitempty"`
	Provider *ConfigureOAuthProvider200ResponseProvider `json:"provider,omitempty"`
}

// NewConfigureOAuthProvider200Response instantiates a new ConfigureOAuthProvider200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewConfigureOAuthProvider200Response() *ConfigureOAuthProvider200Response {
	this := ConfigureOAuthProvider200Response{}
	return &this
}

// NewConfigureOAuthProvider200ResponseWithDefaults instantiates a new ConfigureOAuthProvider200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewConfigureOAuthProvider200ResponseWithDefaults() *ConfigureOAuthProvider200Response {
	this := ConfigureOAuthProvider200Response{}
	return &this
}

// GetMessage returns the Message field value if set, zero value otherwise.
func (o *ConfigureOAuthProvider200Response) GetMessage() string {
	if o == nil || IsNil(o.Message) {
		var ret string
		return ret
	}
	return *o.Message
}

// GetMessageOk returns a tuple with the Message field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConfigureOAuthProvider200Response) GetMessageOk() (*string, bool) {
	if o == nil || IsNil(o.Message) {
		return nil, false
	}
	return o.Message, true
}

// HasMessage returns a boolean if a field has been set.
func (o *ConfigureOAuthProvider200Response) HasMessage() bool {
	if o != nil && !IsNil(o.Message) {
		return true
	}

	return false
}

// SetMessage gets a reference to the given string and assigns it to the Message field.
func (o *ConfigureOAuthProvider200Response) SetMessage(v string) {
	o.Message = &v
}

// GetProvider returns the Provider field value if set, zero value otherwise.
func (o *ConfigureOAuthProvider200Response) GetProvider() ConfigureOAuthProvider200ResponseProvider {
	if o == nil || IsNil(o.Provider) {
		var ret ConfigureOAuthProvider200ResponseProvider
		return ret
	}
	return *o.Provider
}

// GetProviderOk returns a tuple with the Provider field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConfigureOAuthProvider200Response) GetProviderOk() (*ConfigureOAuthProvider200ResponseProvider, bool) {
	if o == nil || IsNil(o.Provider) {
		return nil, false
	}
	return o.Provider, true
}

// HasProvider returns a boolean if a field has been set.
func (o *ConfigureOAuthProvider200Response) HasProvider() bool {
	if o != nil && !IsNil(o.Provider) {
		return true
	}

	return false
}

// SetProvider gets a reference to the given ConfigureOAuthProvider200ResponseProvider and assigns it to the Provider field.
func (o *ConfigureOAuthProvider200Response) SetProvider(v ConfigureOAuthProvider200ResponseProvider) {
	o.Provider = &v
}

func (o ConfigureOAuthProvider200Response) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ConfigureOAuthProvider200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Message) {
		toSerialize["message"] = o.Message
	}
	if !IsNil(o.Provider) {
		toSerialize["provider"] = o.Provider
	}
	return toSerialize, nil
}

type NullableConfigureOAuthProvider200Response struct {
	value *ConfigureOAuthProvider200Response
	isSet bool
}

func (v NullableConfigureOAuthProvider200Response) Get() *ConfigureOAuthProvider200Response {
	return v.value
}

func (v *NullableConfigureOAuthProvider200Response) Set(val *ConfigureOAuthProvider200Response) {
	v.value = val
	v.isSet = true
}

func (v NullableConfigureOAuthProvider200Response) IsSet() bool {
	return v.isSet
}

func (v *NullableConfigureOAuthProvider200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableConfigureOAuthProvider200Response(val *ConfigureOAuthProvider200Response) *NullableConfigureOAuthProvider200Response {
	return &NullableConfigureOAuthProvider200Response{value: val, isSet: true}
}

func (v NullableConfigureOAuthProvider200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableConfigureOAuthProvider200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


