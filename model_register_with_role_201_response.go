/*
MUDBASESDK

MUDBASE is a scalable, real-time, and secure Backend-as-a-Service (BaaS) platform  designed for modern applications. Built with custom logic, it offers fine-grained  control, extensibility, and enterprise-grade security.  ## Features - 🔐 Multi-provider authentication (30+ OAuth providers) - 📊 Real-time database with collections - 📁 File storage and management - 🔑 API key management with permissions - 🔗 Webhook system with retry logic - ⚡ Serverless functions - 💬 Multi-channel messaging (Push, Email, SMS) - 📈 Usage analytics and monitoring - 🌐 Real-time WebSocket events - 🔍 Full-text search capabilities - 💳 Billing: fiat only for project subscriptions and org BaaS checkout (platform fee split). On-chain billing is not exposed on these APIs (optional `crypto-payment-module/` in repo, not mounted by default). - 📡 Block-based multi-chain wallet monitoring (ETH/UTXO scanners, GetBlock, scanner metrics API) - 🏢 Enterprise / Phase 4: custom domains on Growth, Scale, and Enterprise (TXT DNS at `_mudbase-verify.<hostname>`); `settings.customDomainAddon` is optional (billing/legacy); dedicated DB migration script, periodic DNS recheck job, optional `infrastructureEnvironments[]` and edge/metering fields on `dedicated`. 

API version: 1.3.6
Contact: support@mudbase.dev
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package mudbase

import (
	"encoding/json"
)

// checks if the RegisterWithRole201Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &RegisterWithRole201Response{}

// RegisterWithRole201Response struct for RegisterWithRole201Response
type RegisterWithRole201Response struct {
	Message *string `json:"message,omitempty"`
	// True when the project requires email verification before a session is issued - no token is returned in that case.
	RequireVerification *bool `json:"requireVerification,omitempty"`
	// JWT access token. Absent when requireVerification is true.
	Token *string `json:"token,omitempty"`
	// JWT refresh token. Absent when requireVerification is true.
	RefreshToken *string `json:"refreshToken,omitempty"`
	// Access token TTL in seconds. Absent when requireVerification is true.
	ExpiresIn *int32 `json:"expiresIn,omitempty"`
	User *RegisterWithRole201ResponseUser `json:"user,omitempty"`
	Role *RegisterWithRole201ResponseRole `json:"role,omitempty"`
}

// NewRegisterWithRole201Response instantiates a new RegisterWithRole201Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewRegisterWithRole201Response() *RegisterWithRole201Response {
	this := RegisterWithRole201Response{}
	return &this
}

// NewRegisterWithRole201ResponseWithDefaults instantiates a new RegisterWithRole201Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewRegisterWithRole201ResponseWithDefaults() *RegisterWithRole201Response {
	this := RegisterWithRole201Response{}
	return &this
}

// GetMessage returns the Message field value if set, zero value otherwise.
func (o *RegisterWithRole201Response) GetMessage() string {
	if o == nil || IsNil(o.Message) {
		var ret string
		return ret
	}
	return *o.Message
}

// GetMessageOk returns a tuple with the Message field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RegisterWithRole201Response) GetMessageOk() (*string, bool) {
	if o == nil || IsNil(o.Message) {
		return nil, false
	}
	return o.Message, true
}

// HasMessage returns a boolean if a field has been set.
func (o *RegisterWithRole201Response) HasMessage() bool {
	if o != nil && !IsNil(o.Message) {
		return true
	}

	return false
}

// SetMessage gets a reference to the given string and assigns it to the Message field.
func (o *RegisterWithRole201Response) SetMessage(v string) {
	o.Message = &v
}

// GetRequireVerification returns the RequireVerification field value if set, zero value otherwise.
func (o *RegisterWithRole201Response) GetRequireVerification() bool {
	if o == nil || IsNil(o.RequireVerification) {
		var ret bool
		return ret
	}
	return *o.RequireVerification
}

// GetRequireVerificationOk returns a tuple with the RequireVerification field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RegisterWithRole201Response) GetRequireVerificationOk() (*bool, bool) {
	if o == nil || IsNil(o.RequireVerification) {
		return nil, false
	}
	return o.RequireVerification, true
}

// HasRequireVerification returns a boolean if a field has been set.
func (o *RegisterWithRole201Response) HasRequireVerification() bool {
	if o != nil && !IsNil(o.RequireVerification) {
		return true
	}

	return false
}

// SetRequireVerification gets a reference to the given bool and assigns it to the RequireVerification field.
func (o *RegisterWithRole201Response) SetRequireVerification(v bool) {
	o.RequireVerification = &v
}

// GetToken returns the Token field value if set, zero value otherwise.
func (o *RegisterWithRole201Response) GetToken() string {
	if o == nil || IsNil(o.Token) {
		var ret string
		return ret
	}
	return *o.Token
}

// GetTokenOk returns a tuple with the Token field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RegisterWithRole201Response) GetTokenOk() (*string, bool) {
	if o == nil || IsNil(o.Token) {
		return nil, false
	}
	return o.Token, true
}

// HasToken returns a boolean if a field has been set.
func (o *RegisterWithRole201Response) HasToken() bool {
	if o != nil && !IsNil(o.Token) {
		return true
	}

	return false
}

// SetToken gets a reference to the given string and assigns it to the Token field.
func (o *RegisterWithRole201Response) SetToken(v string) {
	o.Token = &v
}

// GetRefreshToken returns the RefreshToken field value if set, zero value otherwise.
func (o *RegisterWithRole201Response) GetRefreshToken() string {
	if o == nil || IsNil(o.RefreshToken) {
		var ret string
		return ret
	}
	return *o.RefreshToken
}

// GetRefreshTokenOk returns a tuple with the RefreshToken field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RegisterWithRole201Response) GetRefreshTokenOk() (*string, bool) {
	if o == nil || IsNil(o.RefreshToken) {
		return nil, false
	}
	return o.RefreshToken, true
}

// HasRefreshToken returns a boolean if a field has been set.
func (o *RegisterWithRole201Response) HasRefreshToken() bool {
	if o != nil && !IsNil(o.RefreshToken) {
		return true
	}

	return false
}

// SetRefreshToken gets a reference to the given string and assigns it to the RefreshToken field.
func (o *RegisterWithRole201Response) SetRefreshToken(v string) {
	o.RefreshToken = &v
}

// GetExpiresIn returns the ExpiresIn field value if set, zero value otherwise.
func (o *RegisterWithRole201Response) GetExpiresIn() int32 {
	if o == nil || IsNil(o.ExpiresIn) {
		var ret int32
		return ret
	}
	return *o.ExpiresIn
}

// GetExpiresInOk returns a tuple with the ExpiresIn field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RegisterWithRole201Response) GetExpiresInOk() (*int32, bool) {
	if o == nil || IsNil(o.ExpiresIn) {
		return nil, false
	}
	return o.ExpiresIn, true
}

// HasExpiresIn returns a boolean if a field has been set.
func (o *RegisterWithRole201Response) HasExpiresIn() bool {
	if o != nil && !IsNil(o.ExpiresIn) {
		return true
	}

	return false
}

// SetExpiresIn gets a reference to the given int32 and assigns it to the ExpiresIn field.
func (o *RegisterWithRole201Response) SetExpiresIn(v int32) {
	o.ExpiresIn = &v
}

// GetUser returns the User field value if set, zero value otherwise.
func (o *RegisterWithRole201Response) GetUser() RegisterWithRole201ResponseUser {
	if o == nil || IsNil(o.User) {
		var ret RegisterWithRole201ResponseUser
		return ret
	}
	return *o.User
}

// GetUserOk returns a tuple with the User field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RegisterWithRole201Response) GetUserOk() (*RegisterWithRole201ResponseUser, bool) {
	if o == nil || IsNil(o.User) {
		return nil, false
	}
	return o.User, true
}

// HasUser returns a boolean if a field has been set.
func (o *RegisterWithRole201Response) HasUser() bool {
	if o != nil && !IsNil(o.User) {
		return true
	}

	return false
}

// SetUser gets a reference to the given RegisterWithRole201ResponseUser and assigns it to the User field.
func (o *RegisterWithRole201Response) SetUser(v RegisterWithRole201ResponseUser) {
	o.User = &v
}

// GetRole returns the Role field value if set, zero value otherwise.
func (o *RegisterWithRole201Response) GetRole() RegisterWithRole201ResponseRole {
	if o == nil || IsNil(o.Role) {
		var ret RegisterWithRole201ResponseRole
		return ret
	}
	return *o.Role
}

// GetRoleOk returns a tuple with the Role field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RegisterWithRole201Response) GetRoleOk() (*RegisterWithRole201ResponseRole, bool) {
	if o == nil || IsNil(o.Role) {
		return nil, false
	}
	return o.Role, true
}

// HasRole returns a boolean if a field has been set.
func (o *RegisterWithRole201Response) HasRole() bool {
	if o != nil && !IsNil(o.Role) {
		return true
	}

	return false
}

// SetRole gets a reference to the given RegisterWithRole201ResponseRole and assigns it to the Role field.
func (o *RegisterWithRole201Response) SetRole(v RegisterWithRole201ResponseRole) {
	o.Role = &v
}

func (o RegisterWithRole201Response) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o RegisterWithRole201Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Message) {
		toSerialize["message"] = o.Message
	}
	if !IsNil(o.RequireVerification) {
		toSerialize["requireVerification"] = o.RequireVerification
	}
	if !IsNil(o.Token) {
		toSerialize["token"] = o.Token
	}
	if !IsNil(o.RefreshToken) {
		toSerialize["refreshToken"] = o.RefreshToken
	}
	if !IsNil(o.ExpiresIn) {
		toSerialize["expiresIn"] = o.ExpiresIn
	}
	if !IsNil(o.User) {
		toSerialize["user"] = o.User
	}
	if !IsNil(o.Role) {
		toSerialize["role"] = o.Role
	}
	return toSerialize, nil
}

type NullableRegisterWithRole201Response struct {
	value *RegisterWithRole201Response
	isSet bool
}

func (v NullableRegisterWithRole201Response) Get() *RegisterWithRole201Response {
	return v.value
}

func (v *NullableRegisterWithRole201Response) Set(val *RegisterWithRole201Response) {
	v.value = val
	v.isSet = true
}

func (v NullableRegisterWithRole201Response) IsSet() bool {
	return v.isSet
}

func (v *NullableRegisterWithRole201Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRegisterWithRole201Response(val *RegisterWithRole201Response) *NullableRegisterWithRole201Response {
	return &NullableRegisterWithRole201Response{value: val, isSet: true}
}

func (v NullableRegisterWithRole201Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRegisterWithRole201Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


