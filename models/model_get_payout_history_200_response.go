/*
MUDBASESDK

MUDBASE is a scalable, real-time, and secure Backend-as-a-Service (BaaS) platform  designed for modern applications. Built with custom logic, it offers fine-grained  control, extensibility, and enterprise-grade security.  ## Features - 🔐 Multi-provider authentication (30+ OAuth providers) - 📊 Real-time database with collections - 📁 File storage and management - 🔑 API key management with permissions - 🔗 Webhook system with retry logic - ⚡ Serverless functions - 💬 Multi-channel messaging (Push, Email, SMS) - 📈 Usage analytics and monitoring - 🌐 Real-time WebSocket events - 🔍 Full-text search capabilities - 💳 Billing: fiat only for project subscriptions and org BaaS checkout (platform fee split). On-chain billing is not exposed on these APIs (optional `crypto-payment-module/` in repo, not mounted by default). - 🏢 Enterprise / Phase 4: custom domains on Growth, Scale, and Enterprise (TXT DNS at `_mudbase-verify.<hostname>`); `settings.customDomainAddon` is optional (billing/legacy); dedicated DB migration script, periodic DNS recheck job, optional `infrastructureEnvironments[]` and edge/metering fields on `dedicated`. 

API version: 1.3.13
Contact: support@mudbase.dev
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package models

import (
	"encoding/json"
)

// checks if the GetPayoutHistory200Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetPayoutHistory200Response{}

// GetPayoutHistory200Response struct for GetPayoutHistory200Response
type GetPayoutHistory200Response struct {
	Success *bool `json:"success,omitempty"`
	Data []GetPayoutHistory200ResponseDataInner `json:"data,omitempty"`
	Pagination *GetPayoutHistory200ResponsePagination `json:"pagination,omitempty"`
}

// NewGetPayoutHistory200Response instantiates a new GetPayoutHistory200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetPayoutHistory200Response() *GetPayoutHistory200Response {
	this := GetPayoutHistory200Response{}
	return &this
}

// NewGetPayoutHistory200ResponseWithDefaults instantiates a new GetPayoutHistory200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetPayoutHistory200ResponseWithDefaults() *GetPayoutHistory200Response {
	this := GetPayoutHistory200Response{}
	return &this
}

// GetSuccess returns the Success field value if set, zero value otherwise.
func (o *GetPayoutHistory200Response) GetSuccess() bool {
	if o == nil || IsNil(o.Success) {
		var ret bool
		return ret
	}
	return *o.Success
}

// GetSuccessOk returns a tuple with the Success field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetPayoutHistory200Response) GetSuccessOk() (*bool, bool) {
	if o == nil || IsNil(o.Success) {
		return nil, false
	}
	return o.Success, true
}

// HasSuccess returns a boolean if a field has been set.
func (o *GetPayoutHistory200Response) HasSuccess() bool {
	if o != nil && !IsNil(o.Success) {
		return true
	}

	return false
}

// SetSuccess gets a reference to the given bool and assigns it to the Success field.
func (o *GetPayoutHistory200Response) SetSuccess(v bool) {
	o.Success = &v
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *GetPayoutHistory200Response) GetData() []GetPayoutHistory200ResponseDataInner {
	if o == nil || IsNil(o.Data) {
		var ret []GetPayoutHistory200ResponseDataInner
		return ret
	}
	return o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetPayoutHistory200Response) GetDataOk() ([]GetPayoutHistory200ResponseDataInner, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *GetPayoutHistory200Response) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given []GetPayoutHistory200ResponseDataInner and assigns it to the Data field.
func (o *GetPayoutHistory200Response) SetData(v []GetPayoutHistory200ResponseDataInner) {
	o.Data = v
}

// GetPagination returns the Pagination field value if set, zero value otherwise.
func (o *GetPayoutHistory200Response) GetPagination() GetPayoutHistory200ResponsePagination {
	if o == nil || IsNil(o.Pagination) {
		var ret GetPayoutHistory200ResponsePagination
		return ret
	}
	return *o.Pagination
}

// GetPaginationOk returns a tuple with the Pagination field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetPayoutHistory200Response) GetPaginationOk() (*GetPayoutHistory200ResponsePagination, bool) {
	if o == nil || IsNil(o.Pagination) {
		return nil, false
	}
	return o.Pagination, true
}

// HasPagination returns a boolean if a field has been set.
func (o *GetPayoutHistory200Response) HasPagination() bool {
	if o != nil && !IsNil(o.Pagination) {
		return true
	}

	return false
}

// SetPagination gets a reference to the given GetPayoutHistory200ResponsePagination and assigns it to the Pagination field.
func (o *GetPayoutHistory200Response) SetPagination(v GetPayoutHistory200ResponsePagination) {
	o.Pagination = &v
}

func (o GetPayoutHistory200Response) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetPayoutHistory200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Success) {
		toSerialize["success"] = o.Success
	}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	if !IsNil(o.Pagination) {
		toSerialize["pagination"] = o.Pagination
	}
	return toSerialize, nil
}

type NullableGetPayoutHistory200Response struct {
	value *GetPayoutHistory200Response
	isSet bool
}

func (v NullableGetPayoutHistory200Response) Get() *GetPayoutHistory200Response {
	return v.value
}

func (v *NullableGetPayoutHistory200Response) Set(val *GetPayoutHistory200Response) {
	v.value = val
	v.isSet = true
}

func (v NullableGetPayoutHistory200Response) IsSet() bool {
	return v.isSet
}

func (v *NullableGetPayoutHistory200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetPayoutHistory200Response(val *GetPayoutHistory200Response) *NullableGetPayoutHistory200Response {
	return &NullableGetPayoutHistory200Response{value: val, isSet: true}
}

func (v NullableGetPayoutHistory200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetPayoutHistory200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


