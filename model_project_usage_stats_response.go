/*
MUDBASESDK

MUDBASE is a scalable, real-time, and secure Backend-as-a-Service (BaaS) platform  designed for modern applications. Built with custom logic, it offers fine-grained  control, extensibility, and enterprise-grade security.  ## Features - 🔐 Multi-provider authentication (30+ OAuth providers) - 📊 Real-time database with collections - 📁 File storage and management - 🔑 API key management with permissions - 🔗 Webhook system with retry logic - ⚡ Serverless functions - 💬 Multi-channel messaging (Push, Email, SMS) - 📈 Usage analytics and monitoring - 🌐 Real-time WebSocket events - 🔍 Full-text search capabilities - 💳 Billing: Flutterwave only for fiat (platform subscriptions + org payment processing with 7% + $0.50 split); crypto payment processor for multi-chain checkout - 📡 Block-based multi-chain wallet monitoring (ETH/UTXO scanners, GetBlock, scanner metrics API) 

API version: 1.2.3
Contact: support@dev.mudbase
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package mudbase

import (
	"encoding/json"
)

// checks if the ProjectUsageStatsResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ProjectUsageStatsResponse{}

// ProjectUsageStatsResponse Project-level usage stats (project, usage, period).
type ProjectUsageStatsResponse struct {
	Project *ProjectUsageStatsResponseProject `json:"project,omitempty"`
	Usage *ProjectUsage `json:"usage,omitempty"`
	Period *string `json:"period,omitempty"`
}

// NewProjectUsageStatsResponse instantiates a new ProjectUsageStatsResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewProjectUsageStatsResponse() *ProjectUsageStatsResponse {
	this := ProjectUsageStatsResponse{}
	return &this
}

// NewProjectUsageStatsResponseWithDefaults instantiates a new ProjectUsageStatsResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewProjectUsageStatsResponseWithDefaults() *ProjectUsageStatsResponse {
	this := ProjectUsageStatsResponse{}
	return &this
}

// GetProject returns the Project field value if set, zero value otherwise.
func (o *ProjectUsageStatsResponse) GetProject() ProjectUsageStatsResponseProject {
	if o == nil || IsNil(o.Project) {
		var ret ProjectUsageStatsResponseProject
		return ret
	}
	return *o.Project
}

// GetProjectOk returns a tuple with the Project field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProjectUsageStatsResponse) GetProjectOk() (*ProjectUsageStatsResponseProject, bool) {
	if o == nil || IsNil(o.Project) {
		return nil, false
	}
	return o.Project, true
}

// HasProject returns a boolean if a field has been set.
func (o *ProjectUsageStatsResponse) HasProject() bool {
	if o != nil && !IsNil(o.Project) {
		return true
	}

	return false
}

// SetProject gets a reference to the given ProjectUsageStatsResponseProject and assigns it to the Project field.
func (o *ProjectUsageStatsResponse) SetProject(v ProjectUsageStatsResponseProject) {
	o.Project = &v
}

// GetUsage returns the Usage field value if set, zero value otherwise.
func (o *ProjectUsageStatsResponse) GetUsage() ProjectUsage {
	if o == nil || IsNil(o.Usage) {
		var ret ProjectUsage
		return ret
	}
	return *o.Usage
}

// GetUsageOk returns a tuple with the Usage field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProjectUsageStatsResponse) GetUsageOk() (*ProjectUsage, bool) {
	if o == nil || IsNil(o.Usage) {
		return nil, false
	}
	return o.Usage, true
}

// HasUsage returns a boolean if a field has been set.
func (o *ProjectUsageStatsResponse) HasUsage() bool {
	if o != nil && !IsNil(o.Usage) {
		return true
	}

	return false
}

// SetUsage gets a reference to the given ProjectUsage and assigns it to the Usage field.
func (o *ProjectUsageStatsResponse) SetUsage(v ProjectUsage) {
	o.Usage = &v
}

// GetPeriod returns the Period field value if set, zero value otherwise.
func (o *ProjectUsageStatsResponse) GetPeriod() string {
	if o == nil || IsNil(o.Period) {
		var ret string
		return ret
	}
	return *o.Period
}

// GetPeriodOk returns a tuple with the Period field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProjectUsageStatsResponse) GetPeriodOk() (*string, bool) {
	if o == nil || IsNil(o.Period) {
		return nil, false
	}
	return o.Period, true
}

// HasPeriod returns a boolean if a field has been set.
func (o *ProjectUsageStatsResponse) HasPeriod() bool {
	if o != nil && !IsNil(o.Period) {
		return true
	}

	return false
}

// SetPeriod gets a reference to the given string and assigns it to the Period field.
func (o *ProjectUsageStatsResponse) SetPeriod(v string) {
	o.Period = &v
}

func (o ProjectUsageStatsResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ProjectUsageStatsResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Project) {
		toSerialize["project"] = o.Project
	}
	if !IsNil(o.Usage) {
		toSerialize["usage"] = o.Usage
	}
	if !IsNil(o.Period) {
		toSerialize["period"] = o.Period
	}
	return toSerialize, nil
}

type NullableProjectUsageStatsResponse struct {
	value *ProjectUsageStatsResponse
	isSet bool
}

func (v NullableProjectUsageStatsResponse) Get() *ProjectUsageStatsResponse {
	return v.value
}

func (v *NullableProjectUsageStatsResponse) Set(val *ProjectUsageStatsResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableProjectUsageStatsResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableProjectUsageStatsResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableProjectUsageStatsResponse(val *ProjectUsageStatsResponse) *NullableProjectUsageStatsResponse {
	return &NullableProjectUsageStatsResponse{value: val, isSet: true}
}

func (v NullableProjectUsageStatsResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableProjectUsageStatsResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


