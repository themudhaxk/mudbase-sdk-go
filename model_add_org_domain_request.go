/*
MUDBASESDK

MUDBASE is a scalable, real-time, and secure Backend-as-a-Service (BaaS) platform  designed for modern applications. Built with custom logic, it offers fine-grained  control, extensibility, and enterprise-grade security.  ## Features - 🔐 Multi-provider authentication (30+ OAuth providers) - 📊 Real-time database with collections - 📁 File storage and management - 🔑 API key management with permissions - 🔗 Webhook system with retry logic - ⚡ Serverless functions - 💬 Multi-channel messaging (Push, Email, SMS) - 📈 Usage analytics and monitoring - 🌐 Real-time WebSocket events - 🔍 Full-text search capabilities - 💳 Billing: fiat only for project subscriptions and org BaaS checkout (platform fee split). On-chain billing is not exposed on these APIs (optional `crypto-payment-module/` in repo, not mounted by default). - 📡 Block-based multi-chain wallet monitoring (ETH/UTXO scanners, GetBlock, scanner metrics API) - 🏢 Enterprise / Phase 4: custom domains on Growth, Scale, and Enterprise (TXT DNS at `_mudbase-verify.<hostname>`); `settings.customDomainAddon` is optional (billing/legacy); dedicated DB migration script, periodic DNS recheck job, optional `infrastructureEnvironments[]` and edge/metering fields on `dedicated`. 

API version: 1.3.5
Contact: support@mudbase.dev
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package mudbase

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the AddOrgDomainRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AddOrgDomainRequest{}

// AddOrgDomainRequest struct for AddOrgDomainRequest
type AddOrgDomainRequest struct {
	Hostname string `json:"hostname"`
	SetPrimary *bool `json:"setPrimary,omitempty"`
}

type _AddOrgDomainRequest AddOrgDomainRequest

// NewAddOrgDomainRequest instantiates a new AddOrgDomainRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAddOrgDomainRequest(hostname string) *AddOrgDomainRequest {
	this := AddOrgDomainRequest{}
	this.Hostname = hostname
	return &this
}

// NewAddOrgDomainRequestWithDefaults instantiates a new AddOrgDomainRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAddOrgDomainRequestWithDefaults() *AddOrgDomainRequest {
	this := AddOrgDomainRequest{}
	return &this
}

// GetHostname returns the Hostname field value
func (o *AddOrgDomainRequest) GetHostname() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Hostname
}

// GetHostnameOk returns a tuple with the Hostname field value
// and a boolean to check if the value has been set.
func (o *AddOrgDomainRequest) GetHostnameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Hostname, true
}

// SetHostname sets field value
func (o *AddOrgDomainRequest) SetHostname(v string) {
	o.Hostname = v
}

// GetSetPrimary returns the SetPrimary field value if set, zero value otherwise.
func (o *AddOrgDomainRequest) GetSetPrimary() bool {
	if o == nil || IsNil(o.SetPrimary) {
		var ret bool
		return ret
	}
	return *o.SetPrimary
}

// GetSetPrimaryOk returns a tuple with the SetPrimary field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AddOrgDomainRequest) GetSetPrimaryOk() (*bool, bool) {
	if o == nil || IsNil(o.SetPrimary) {
		return nil, false
	}
	return o.SetPrimary, true
}

// HasSetPrimary returns a boolean if a field has been set.
func (o *AddOrgDomainRequest) HasSetPrimary() bool {
	if o != nil && !IsNil(o.SetPrimary) {
		return true
	}

	return false
}

// SetSetPrimary gets a reference to the given bool and assigns it to the SetPrimary field.
func (o *AddOrgDomainRequest) SetSetPrimary(v bool) {
	o.SetPrimary = &v
}

func (o AddOrgDomainRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AddOrgDomainRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["hostname"] = o.Hostname
	if !IsNil(o.SetPrimary) {
		toSerialize["setPrimary"] = o.SetPrimary
	}
	return toSerialize, nil
}

func (o *AddOrgDomainRequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"hostname",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varAddOrgDomainRequest := _AddOrgDomainRequest{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varAddOrgDomainRequest)

	if err != nil {
		return err
	}

	*o = AddOrgDomainRequest(varAddOrgDomainRequest)

	return err
}

type NullableAddOrgDomainRequest struct {
	value *AddOrgDomainRequest
	isSet bool
}

func (v NullableAddOrgDomainRequest) Get() *AddOrgDomainRequest {
	return v.value
}

func (v *NullableAddOrgDomainRequest) Set(val *AddOrgDomainRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableAddOrgDomainRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableAddOrgDomainRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAddOrgDomainRequest(val *AddOrgDomainRequest) *NullableAddOrgDomainRequest {
	return &NullableAddOrgDomainRequest{value: val, isSet: true}
}

func (v NullableAddOrgDomainRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAddOrgDomainRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


