/*
MUDBASESDK

MUDBASE is a scalable, real-time, and secure Backend-as-a-Service (BaaS) platform  designed for modern applications. Built with custom logic, it offers fine-grained  control, extensibility, and enterprise-grade security.  ## Features - 🔐 Multi-provider authentication (30+ OAuth providers) - 📊 Real-time database with collections - 📁 File storage and management - 🔑 API key management with permissions - 🔗 Webhook system with retry logic - ⚡ Serverless functions - 💬 Multi-channel messaging (Push, Email, SMS) - 📈 Usage analytics and monitoring - 🌐 Real-time WebSocket events - 🔍 Full-text search capabilities - 💳 Billing: Flutterwave only for fiat (platform subscriptions + org payment processing with 7% + $0.50 split); crypto payment processor for multi-chain checkout - 📡 Block-based multi-chain wallet monitoring (ETH/UTXO scanners, GetBlock, scanner metrics API) 

API version: 1.2.3
Contact: support@dev.mudbase
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package mudbase

import (
	"encoding/json"
)

// checks if the WalletUpdateWebhook200Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &WalletUpdateWebhook200Response{}

// WalletUpdateWebhook200Response struct for WalletUpdateWebhook200Response
type WalletUpdateWebhook200Response struct {
	Success *bool `json:"success,omitempty"`
	Data *WalletWebhook `json:"data,omitempty"`
}

// NewWalletUpdateWebhook200Response instantiates a new WalletUpdateWebhook200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewWalletUpdateWebhook200Response() *WalletUpdateWebhook200Response {
	this := WalletUpdateWebhook200Response{}
	return &this
}

// NewWalletUpdateWebhook200ResponseWithDefaults instantiates a new WalletUpdateWebhook200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewWalletUpdateWebhook200ResponseWithDefaults() *WalletUpdateWebhook200Response {
	this := WalletUpdateWebhook200Response{}
	return &this
}

// GetSuccess returns the Success field value if set, zero value otherwise.
func (o *WalletUpdateWebhook200Response) GetSuccess() bool {
	if o == nil || IsNil(o.Success) {
		var ret bool
		return ret
	}
	return *o.Success
}

// GetSuccessOk returns a tuple with the Success field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WalletUpdateWebhook200Response) GetSuccessOk() (*bool, bool) {
	if o == nil || IsNil(o.Success) {
		return nil, false
	}
	return o.Success, true
}

// HasSuccess returns a boolean if a field has been set.
func (o *WalletUpdateWebhook200Response) HasSuccess() bool {
	if o != nil && !IsNil(o.Success) {
		return true
	}

	return false
}

// SetSuccess gets a reference to the given bool and assigns it to the Success field.
func (o *WalletUpdateWebhook200Response) SetSuccess(v bool) {
	o.Success = &v
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *WalletUpdateWebhook200Response) GetData() WalletWebhook {
	if o == nil || IsNil(o.Data) {
		var ret WalletWebhook
		return ret
	}
	return *o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WalletUpdateWebhook200Response) GetDataOk() (*WalletWebhook, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *WalletUpdateWebhook200Response) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given WalletWebhook and assigns it to the Data field.
func (o *WalletUpdateWebhook200Response) SetData(v WalletWebhook) {
	o.Data = &v
}

func (o WalletUpdateWebhook200Response) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o WalletUpdateWebhook200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Success) {
		toSerialize["success"] = o.Success
	}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	return toSerialize, nil
}

type NullableWalletUpdateWebhook200Response struct {
	value *WalletUpdateWebhook200Response
	isSet bool
}

func (v NullableWalletUpdateWebhook200Response) Get() *WalletUpdateWebhook200Response {
	return v.value
}

func (v *NullableWalletUpdateWebhook200Response) Set(val *WalletUpdateWebhook200Response) {
	v.value = val
	v.isSet = true
}

func (v NullableWalletUpdateWebhook200Response) IsSet() bool {
	return v.isSet
}

func (v *NullableWalletUpdateWebhook200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableWalletUpdateWebhook200Response(val *WalletUpdateWebhook200Response) *NullableWalletUpdateWebhook200Response {
	return &NullableWalletUpdateWebhook200Response{value: val, isSet: true}
}

func (v NullableWalletUpdateWebhook200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableWalletUpdateWebhook200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


